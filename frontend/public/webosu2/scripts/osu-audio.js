define([], function () {
  ...
  function OsuAudio(filename, buffer, callback) {
    ...
    this.play = function play(wait = 0) {
      ...
      // Reset the gain to 1.0 in case a previous play scheduled a fade-out
      // (e.g. the death animation ramps gain → 0 over ~1s). The GainNode
      // is shared across plays, so without cancelling its scheduled values
      // and re-snapping to 1.0, subsequent plays would be silent.
      try {
        var _now = self.audio.currentTime;
        self.gain.gain.cancelScheduledValues(_now);
        self.gain.gain.setValueAtTime(1.0, _now);
      } catch (_) {}
      self.playing = true;
