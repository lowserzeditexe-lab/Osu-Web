function setOptionPanel() {
  ...
  window.gamesettings.loadToGame = function () {
    if (window.game) {
      ...
      window.game.sudden = this.sudden;
      window.game.perfect = this.perfect;
      window.game.flashlight = this.flashlight;
  ...
  function bindExclusiveCheck4(id1, item1, id2, item2, id3, item3, id4, item4) {
    let c1 = document.getElementById(id1);
    let c2 = document.getElementById(id2);
    let c3 = document.getElementById(id3);
    let c4 = document.getElementById(id4);
    let cs = [c1, c2, c3, c4];
    let items = [item1, item2, item3, item4];
    cs.forEach((c, i) => { c.checked = gamesettings[items[i]]; });
    gamesettings.restoreCallbacks.push(function () {
      cs.forEach((c, i) => { c.checked = gamesettings[items[i]]; checkdefault(c, items[i]); });
    });
    cs.forEach((c, i) => { checkdefault(c, items[i]); });
    cs.forEach((c, i) => {
      c.onclick = function () {
        // Selecting one clears the others.
        for (let j = 0; j < 4; j++) {
          if (j === i) continue;
          gamesettings[items[j]] = false;
          cs[j].checked = false;
          checkdefault(cs[j], items[j]);
        }
        gamesettings[items[i]] = c.checked;
        gamesettings.loadToGame();
        saveToLocal();
        checkdefault(c, items[i]);
      };
    });
  }
  ...
  // Speed mods are pairwise mutually exclusive (HT/DC/DT/NC).
  bindExclusiveCheck4(
  ...
  // Fail mods are pairwise mutually exclusive (NF/SD/PF).
  bindExclusiveCheck3(
    "nofail-check", "nofail",
    "sudden-check", "sudden",
    "perfect-check", "perfect"
  );
  bindExclusiveCheck3(
    "relax-check",
    "relax",
  ...
  bindcheck("flashlight-check", "flashlight");
