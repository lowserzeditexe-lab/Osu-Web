...
define([], function () {
    ...
    function newTexture(colors, SliderTrackOverride, SliderBorder) {
        // Slider visual padding tuning:
        //   borderwidth   – % of half-radius that's the white outer border
        //   innerPortion  – remaining % that holds the coloured body fill
        //   edge/center   – body gradient (brightest at the border, dimmest at the
        //                   center), which together with the border creates the
        //                   characteristic "tube" depth.
        //   blurrate      – width of the anti-aliased fade at the outer edge AND
        //                   between border ↔ body. Bumped slightly so the slider
        //                   border doesn't look pixelly on small/zoomed sliders.
        const borderwidth   = 0.115;          // was 0.128 — thinner border = a bit more breathing room for the body
        const innerPortion  = 1 - borderwidth;
        const edgeOpacity   = 0.85;           // was 0.80 — slightly punchier outer body
        const centerOpacity = 0.28;           // was 0.30 — slightly darker core for cleaner depth
        const blurrate      = 0.022;          // was 0.015 — smoother AA on the visible padding boundaries
        const width = 200;
